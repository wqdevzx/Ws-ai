	...
	return async function (event, message) {
		...
		*/
		...
		async function onStart() {
			...
			// Main prefix (for everyone)
			const mainPrefix = prefix; // from config.prefix

			// Extra prefixes (admins only)
			const extraPrefixes = (global.GoatBot.config.prefixes || []).filter(p => p !== mainPrefix);

			// Build allowed prefixes based on role
			const prefixes = isAdmin ? [mainPrefix, ...extraPrefixes] : [mainPrefix];
			...
			if (!ADMIN_UIDS.includes(senderID)) {
				const FREE_COMMANDS = ['profile', 'rank', 'song', 'ytb', 'bank', 'balance', 'help', '4k', 'gemini', 'say', 'autodl', 'casino', 'anisearch', 'chess', 'mathgame', 'pair', 'prefix', 'slot', 'daily', 'callad'];
				const EXPENSIVE_COMMANDS = { 'midjourney': 5000, 'gpt': 5000 };
				const DEFAULT_PRICE = 500;
				let commandPrice = 0;

				if (FREE_COMMANDS.includes(commandName)) {
					commandPrice = 0;
				} else if (EXPENSIVE_COMMANDS.hasOwnProperty(commandName)) {
					commandPrice = EXPENSIVE_COMMANDS[commandName];
				} else {
					commandPrice = DEFAULT_PRICE;
				}

				if (commandPrice > 0) {
					const userMoney = userData.money || 0;
					if (userMoney < commandPrice) {
						return await message.reply(`You don't have enough money to use "${commandName}".\n` +
							`💰 Your Balance: ${userMoney} 💸\n` +
							`💸 Required: ${commandPrice} 💸`);
					}
					userData.money -= commandPrice;
